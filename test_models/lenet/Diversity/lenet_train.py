...
def evaluate_model(model, x_img, x_ha, y_gaze):
    scores = []
    for i, (x_im, x_h, y_g) in enumerate(zip(x_img, x_ha, y_gaze)):
        # Add an extra dimension to each input and the output to simulate a batch size of 1
        x_im_batch = np.expand_dims(x_im, axis=0)  # Expand dimensions to create a batch
        x_h_batch = np.expand_dims(x_h, axis=0)
        y_g_batch = np.expand_dims(y_g, axis=0)
        
        # Evaluate the model on the newly created batch
        single_score = model.evaluate([x_im_batch, x_h_batch], y_g_batch, verbose=0)
        scores.append([i, single_score[0], single_score[1]])  # index, loss, accuracy or other metrics
    return scores
...
def main(model_location):
    ...
    if not (os.path.exists(model_location)):
        ...
        # score = model.evaluate([x_img_train, x_ha_train], y_gaze_train, verbose=0)
        # print("score:" + str(score))

        d_scores = evaluate_model(model, x_img_train, x_ha_train, y_gaze_train)
        print("length of the scores: %s" % len(d_scores))
