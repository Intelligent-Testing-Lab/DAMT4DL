...
def train_model(model_loc, model, args, x_train, x_valid, y_train, y_valid):
    ...
    d_scores = evlaute_model(model, validation_generator)
...
def evlaute_model(model, generator):
    """
...
def main(model_loc):
    ...
    if not os.path.exists(model_loc):
        ...
        d_scores = train_model(model_loc, model, args, *data)
    else:
        print('model exists, loading model')
        model = tensorflow.keras.models.load_model(model_loc)
        # metric_value = model.evaluate_generator(validation_generator, verbose=1)
