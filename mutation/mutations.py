...
from mutation import mutation_utils as mu
...
class Mutation():
    ...
    def get_model_params_td(self, elem):
        ...
        if hasattr(elem.value, 'args') and len(elem.value.args) > 0:
            ...
            logging.error("Mutation.get_model_params_td AST node does not have arguments or keywords")
    ...
    def get_model_params_hp(self, elem):
        ...
        if hasattr(elem.value, 'keywords') and len(elem.value.keywords) > 0:
            ...
            logging.error("Mutation.get_model_params_hp AST node does not have keywords")
    ...
    def add_keyword(self, elem, kwd_name, kwd_value):
        try:
            ...
            logging.error("Mutation.add_keyword adding keyword to AST node failed")
    ...
    def mutate(self, file_path, save_path_mutated):
        ...
        try:
            ...
            logging.error("Mutation.mutate parse AST tree failed" + str(e))
            raise

        try:
            was_annotated = self.mutate_annotated(tree, save_path_mutated)
        except Exception as e:
            logging.error("Mutation.mutate_annotated failed:" + str(e))

        try:
            if not was_annotated:
                self.mutate_automatically(tree, save_path_mutated)
        except Exception as e:
            logging.error("Mutation.mutate_automatically failed" + str(e))
            ...
            if hasattr(node, 'body') and isinstance(node.body, list):
                for ind, x in enumerate(node.body):
                    ...
                    #
                        ...
                        save_path_mutated_cnt = os.path.join(save_path_mutated, 'mutated'+str(application_cnt) + '.py')
